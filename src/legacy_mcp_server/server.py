...
try:
    ...
    from .mcp_mock import Server as FastMCP, Tool, ToolResult
    # Create mock types
    class TextContent:
...
class ScholarsQuillServer:
    ...
    def __init__(self, config: Optional[ServerConfig] = None):
        ...
        self.server = FastMCP("scholarsquill")
    ...
    async def _extract_single_file_content(
        ...
        try:
            ...
            # Generate suggested filename using citekey
            metadata = content_data.get("metadata", {})
            citekey = metadata.get("citekey", "unknown_paper")
            suggested_filename = f"{citekey}.md"
        ...
        try:
            ...
            for pdf_file in pdf_files:
                try:
                    ...
                    # Generate suggested filename using citekey
                    metadata = content_data.get("metadata", {})
                    citekey = metadata.get("citekey", pdf_file.stem)
                    suggested_filename = f"{citekey}.md"
...
Please proceed with your analysis using the PDF content, template, and comprehensive instructions provided."""
    ...
    def _generate_codelang_filename(self, metadata: Dict[str, Any], focus: str) -> str:
        """Generate filename for single codelang analysis: [citekey]_codelang.md"""
        citekey = metadata.get("citekey", "unknown_paper")
        # Clean citekey for filename
        safe_citekey = "".join(c for c in citekey if c.isalnum() or c in ('_', '-'))
        return f"{safe_citekey}_codelang.md"
